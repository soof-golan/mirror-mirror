...
def frame_message(sample_frame_jpeg: bytes) -> FrameMessage:
    ...
    return FrameMessage(frame=encode_bytes(sample_frame_jpeg), timestamp=time.time(), camera_id=0)
...
def latents_message(sample_latents: np.ndarray) -> LatentsMessage:
    ...
    return LatentsMessage(latents=data, shape=shape, dtype=dtype, timestamp=time.time(), source="camera")


@pytest.fixture
def prompt_message() -> PromptMessage:
    """Create a sample PromptMessage"""
    return PromptMessage(prompt="a beautiful landscape with mountains and trees", timestamp=time.time())


@pytest.fixture
def processed_frame_message(sample_frame_jpeg: bytes) -> ProcessedFrameMessage:
    """Create a sample ProcessedFrameMessage"""
    return ProcessedFrameMessage(frame=encode_bytes(sample_frame_jpeg), timestamp=time.time(), processing_time=0.123)
