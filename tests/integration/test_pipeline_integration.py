...
from mirror_mirror import encode_frame, decode_frame
...
class TestPipelineDataFlow:
    ...
    def test_frame_to_latents_flow(self, sample_frame):
        ...
        frame_message = FrameMessage(frame=encode_bytes(encoded_frame), timestamp=time.time(), camera_id=0)
        ...
        latents_message = LatentsMessage(
            latents=latents_data, shape=shape, dtype=dtype, timestamp=frame_message.timestamp, source="camera"
        )

        # Step 4: Verify latents can be deserialized (like diffusion_server would)
        reconstructed_latents = deserialize_array(latents_message.latents, latents_message.shape, latents_message.dtype)
    ...
    def test_latents_to_processed_frame_flow(self, sample_frame):
        ...
        diffused_latents_message = LatentsMessage(
            latents=latents_data, shape=shape, dtype=dtype, timestamp=time.time(), source="diffusion"
        )

        # Step 2: Simulate latent decoding (like latent_decoder would)
        latents_array = deserialize_array(
            diffused_latents_message.latents, diffused_latents_message.shape, diffused_latents_message.dtype
        )
    ...
    def test_complete_pipeline_simulation(self, sample_frame):
        ...
        encoded_frame = encode_frame(sample_frame)
        frame_msg = FrameMessage(frame=encode_bytes(encoded_frame), timestamp=start_time, camera_id=0)
        ...
        camera_latents_msg = LatentsMessage(
            latents=latents_data, shape=shape, dtype=dtype, timestamp=frame_msg.timestamp, source="camera"
        )
        ...
        input_latents = deserialize_array(
            camera_latents_msg.latents, camera_latents_msg.shape, camera_latents_msg.dtype
        )
        ...
        output_latents = deserialize_array(
            diffused_latents_msg.latents, diffused_latents_msg.shape, diffused_latents_msg.dtype
        )
...
class TestMessageCarrierIntegration:
    ...
    def test_carrier_message_type_discrimination(self):
        ...
        frame_msg = FrameMessage(frame="test_frame_data", timestamp=time.time(), camera_id=0)

        latents_msg = LatentsMessage(
            latents="test_latents_data", shape=(1, 4, 64, 64), dtype="float32", timestamp=time.time(), source="camera"
        )
...
class TestPerformanceIntegration:
    ...
    def test_pipeline_performance_simulation(self, sample_frame, performance_monitor):
        ...
        for i in range(10):
            # Camera encoding
            encoded_frame = encode_frame(sample_frame)
            frame_msg = FrameMessage(frame=encode_bytes(encoded_frame), timestamp=time.time(), camera_id=0)
            ...
            processed_msg = ProcessedFrameMessage(
                frame=encode_bytes(encoded_final), timestamp=frame_msg.timestamp, processing_time=0.1
            )
    ...
    def test_memory_usage_simulation(self, sample_frame):
        ...
        for i in range(100):
            ...
            frame_msg = FrameMessage(frame=encode_bytes(encoded), timestamp=time.time(), camera_id=0)
            ...
            latents_data, shape, dtype = serialize_array(mock_latents)
            latents_msg = LatentsMessage(
                latents=latents_data, shape=shape, dtype=dtype, timestamp=time.time(), source="camera"
            )
